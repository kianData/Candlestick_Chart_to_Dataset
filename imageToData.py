...
#We need them to cut unused ends of img_data
def unused_ends():
...
def hloc_pos():
    startEnd = start_end()
    data_new = img_data_trim()
    hloc = np.zeros((startEnd.shape[0], 4), dtype=int)
    
    for i in range(0, hloc.shape[0]):
        if (min(data_new[startEnd[i, 0]:startEnd[i, 1], 0])!=-1): 
           hloc[i, 0] = min(data_new[startEnd[i, 0]:startEnd[i, 1], 1]) #Red_Highest
           hloc[i, 1] = max(data_new[startEnd[i, 0]:startEnd[i, 1], 0]) #Red_Lowest
           hloc[i, 2] = max(data_new[startEnd[i, 0]:startEnd[i, 1], 1]) #Red_Open
           hloc[i, 3] = min(data_new[startEnd[i, 0]:startEnd[i, 1], 0]) #Red_Close
        else:
           hloc[i, 0] = min(data_new[startEnd[i, 0]:startEnd[i, 1], 2]) #Green_Highest
           hloc[i, 1] = max(data_new[startEnd[i, 0]:startEnd[i, 1], 1]) #Green_Lowest
           hloc[i, 2] = min(data_new[startEnd[i, 0]:startEnd[i, 1], 1]) #Green_Open
           hloc[i, 3] = max(data_new[startEnd[i, 0]:startEnd[i, 1], 2]) #Green_Close
...
def hloc_values(upper_gridLine_value, lowe_gridLine_value):
    ...
    hlocVal = ((lowe_gridLine_value-upper_gridLine_value)/(hLinesY[-1]-hLinesY[0]))*(hloc-hLinesY[0]) + upper_gridLine_value
...
def date_table(yearLeft, monthLeft, dayLeft):
    ...
    while len(dateTab) < hloc.shape[0]:
